...
# 宏观风险等级
RISK_LEVELS = {
    'LOW': {'max_positions': 5, 'position_pct': 0.25, 'signal_threshold': 3, 'atr_multiplier': 2.0},
    'MEDIUM': {'max_positions': 5, 'position_pct': 0.20, 'signal_threshold': 4, 'atr_multiplier': 1.5},
    'HIGH': {'max_positions': 3, 'position_pct': 0.10, 'signal_threshold': 5, 'atr_multiplier': 1.2},
...
class MacroFilter:
    ...
    def analyze_news_sentiment(self, news_list):
        ...
        # 确定风险等级（修复：避免少量信号触发极端判定）
        # EXTREME 仅当出现多个极端恐慌关键词，或明显看空且有足够样本支撑
        if extreme_fear_count >= 2 or (score < -0.6 and total_signals >= 3):
            risk_level = 'EXTREME'
            reason = f'检测到{extreme_fear_count}个极端恐慌信号，情绪分数{score:.2f}'
        elif extreme_fear_count == 1 or score < -0.3:
            risk_level = 'HIGH'
